...
import net.mcreator.thechaosmod.item.NatureDimensionItem;
...
public class NatureDimensionDimension extends TheChaosModModElements.ModElement {
	@ObjectHolder("the_chaos_mod:nature_dimension_portal")
	public static final CustomPortalBlock portal = null;
	public NatureDimensionDimension(TheChaosModModElements instance) {
		super(instance, 741);
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("the_chaos_mod:nature_dimension"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("nature_dimension_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("nature_dimension_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("nature_dimension_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new NatureDimensionItem().setRegistryName("nature_dimension"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("the_chaos_mod:nature_dimension"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("the_chaos_mod:nature_dimension")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == Blocks.DIRT.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? Blocks.DIRT.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
