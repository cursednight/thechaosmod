...
import net.mcreator.thechaosmod.item.ThanierItem;
...
public class ThanierDimension extends TheChaosModModElements.ModElement {
	@ObjectHolder("the_chaos_mod:thanier_portal")
	public static final CustomPortalBlock portal = null;
	public ThanierDimension(TheChaosModModElements instance) {
		super(instance, 4);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(IonizedThaniumBlock.block.getDefaultState().getBlock()).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("the_chaos_mod:thanier"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("thanier_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("thanier_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("thanier_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new ThanierItem().setRegistryName("thanier"));
	}
		...
		public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					...
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("the_chaos_mod:thanier")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == ThaniumBlock.block.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? ThaniumBlock.block.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, ThaniumBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
